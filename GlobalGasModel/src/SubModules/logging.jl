...
const ggm_status = CustomLogLevel(750, "Status", :light_green)
const ggm_warning = CustomLogLevel(1000, "Warning", :light_yellow)
const ggm_error = CustomLogLevel(2000, "Error", :light_red)
const ggm_check = CustomLogLevel(500, "Check Info", :blue)
...
function handle_message(logger::ConsoleLogger, level::CustomLogLevel, message, _module, group, id, filepath, line; kwargs...)
    # @nospecialize
    ...
    printstyled(iob, offset, "┌ ", levelstr, ": ", msg1, '\n', ;bold = true, color = level.color)
    length(logfile) > 0 && printstyled(file, offset, "┌ ", levelstr, ": ", msg1, '\n', ;bold = true, color = level.color)
    
    for msg in rest
        printstyled(iob, offset, "│ ", msg, '\n', ;bold = true, color = level.color)
        length(logfile) > 0 && printstyled(file, offset, "│ ", msg, '\n', ;bold = true, color = level.color)
    end
    ...
    printstyled(iob, offset, "└ @ ", _module, " ", filepath, ":", line, '\n', ;bold = true, color = level.color)
    length(logfile) > 0 && printstyled(file, offset, "└ @ ", _module, " ", filepath, ":", line, '\n', ;bold = true, color = level.color)
...
macro CheckInfo(exs...)
    ...
    Expr(:macrocall, Base.CoreLogging.var"@logmsg", location, ggm_check, message, kwargs...)
end
...
macro Status(exs...)
    ...
    message = esc(exs[1])
    kwargs = map(maybe_make_kwargs, exs[2:end])
    Expr(:macrocall, Base.CoreLogging.var"@logmsg", location, ggm_status, message, kwargs...)
end
...
macro Warning(exs...)
    ...
    message = esc(exs[1])
    kwargs = map(maybe_make_kwargs, exs[2:end])
    Expr(:macrocall, Base.CoreLogging.var"@logmsg", location, ggm_warning, message, kwargs...)
end
...
macro GGMError(exs...)
    ...
    message = esc(exs[1])
    kwargs = map(maybe_make_kwargs, exs[2:end])
    Expr(:macrocall, Base.CoreLogging.var"@logmsg", location, ggm_error, message, kwargs...)
end